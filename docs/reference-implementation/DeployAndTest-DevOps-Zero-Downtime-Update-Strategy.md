...
In many customer systems there is an assumption that application updates are more frequent than infrastructure updates, and, as such, there are different update procedures for each. Within a public cloud infrastructure, these changes can happen at a much faster pace and it is this and the rate of change on the Azure platform that led the Azure Mission-Critical reference implementation to utilize only one deployment process whether application or infrastructure. This allows:
...
A foundation of the Azure Mission-Critical update strategy is around how branches are used in the Git repository. The Azure Mission-Critical reference implementation uses 3 types of branches:
...
As already described in the previous section, the Azure Mission-Critical reference implementation uses two types of environment: short-lived and permanent.
...
It is important to understand the different types of resources that exist in the Azure Mission-Critical reference implementation deployment for the permanent environments (`int` and `prod`). These are either globally shared resources or dedicated to a particular release and exist only until the next release unit has taken over.
