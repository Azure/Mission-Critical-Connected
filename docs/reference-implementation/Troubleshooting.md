...
It is inevitable in a system with the complexity of Azure Mission-Critical that issues and errors occur. This living document maintains a list of solutions for common errors, which are not directly caused by the Azure Mission-Critical reference implementation code (i.e. are outside of control of the development team and cannot be fixed as bugs in the codebase).
